...
//Please, do not remove this lines!
	...
	private ItemName(final String name) {
		this.name = name;
	}
	
	//Returns the Material from the given item
	public static Material getMaterial(String arg) {
		for (ItemName itemName : values()) {
			if (itemName.name.equalsIgnoreCase(arg) || itemName.name.replace(" ", "").equalsIgnoreCase(arg)) {
				return Material.valueOf(itemName.name());
			}
		}
		return null;
	}
	...
	public static String getNiceName(Material material) {
		return valueOf(material.name()).firstAllUpperCased();
	}
